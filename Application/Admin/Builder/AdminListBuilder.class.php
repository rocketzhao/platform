...
class AdminListBuilder extends AdminBuilder
{
    ...
    public function keyLink($name, $title, $getUrl)
    {
	   ...
	   public function orderStatus($name = 'status', $title = '状态')
    {
        $map = array(-1 => '删除', 0 => '申请报名', 1 => '已经付费', 2 => '签到', 3=>'金额不对');
        return $this->key($name, $title, 'status', $map);
    }

     public function VipStatus($name = 'status', $title = '状态')
    {
        $map = array(-1 => '忽略', 0 => '未生效', 1 => '已生效', 2 => '签到', 3=>'金额不对');
        return $this->key($name, $title, 'status', $map);
    }
