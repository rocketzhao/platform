...
function create(file, $fileInput) {
    var box = $fileInput.parents('.weibo_post_box').find('.parentFileBox');('.FileBox');
    if (box.length <= 0) {
        var div = '<div class="FileBox am-fl">\
        <div class="waitbox">\
       <img src= '+_LOADING_+'  style="width: 100px;height: 100px;"> \
       </div> \
        </div>';
        $fileInput.after(div);
        box = $fileInput.next('FileBox');
    }
}
...
function createBox(result, $fileInput) {
    ...
    var $parentFileBox = $fileInput.parents('.weibo_post_box').find('.parentFileBox');
    var src = result.result.data.file.path;
