...
class VipController extends BaseController
{
    ...
    public function index()
    {
		...
		$vip=M('vipcard_type')->where(array('status'=>1))->select();
		//dump($vip);
        $this->assign("vip", $vip);
...
//dump($issuedetail);exit;
    ...
    {
		$lesson = D('Issue')->where(array('status' => 1, 'title' => '学校课程'))->find(); 
	...
	public function doSendorder()
    {
       ...
       // 
	    $data['vipfee'] = $vipfee = I('post.vipfee', '', 'op_t');       //vipfee
		$map_profiler['vipfee']=$vipfee;
		
		$vipcard=M('vipcard_type')->where($map_profiler)->find();
	//	dump(M()->getlastsql());
		$vipcard['orderid'] = date("YmdHis").$auid; 
		$vipcard['payment'] = I('post.payment', '', 'op_t'); 
		
		 //优惠信息 
		$mobapi = new MobApi();
		$vipinfo=$mobapi->get_vipcardinfo();
		//mp($vipinfo);
		$vipcard['sum']= $vipinfo['count']*1+$vipcard['value']*1;
        $vipcard['before']=$vipinfo['count']*1;
		//ump($vipcard);
		...
		public function confirmorder()
    {
        ...
        $data['uid'] = $auid = get_uid();   
	    $data['vipfee'] = I('post.vipfee', '', 'op_t');       //vipfee
		$data['orderid'] = I('post.orderid', '', 'op_t');
		$data['payment'] = I('post.payment', '', 'op_t'); 
		$data['vipid'] = I('post.vipid', '', 'op_t'); 
		$data['title'] = I('post.title', '', 'op_t'); 
	    $data['vipcnt'] = I('post.vipcnt', '', 'op_t'); 
		
	   $rs=D('vipcard_order')->add($data);
	   
	   $rs=M('vipcard_order')->where(array('orderid'=>$data['orderid']))->count();
