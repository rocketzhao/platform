...
class UserController extends BaseController
{
	    ...
	    public function main()
    {
        ...
        $this->assign('top_menu_list', $this->_top_menu_list);
		//img span
		$img=M('main_page')->where(array('etype' => 'img', 'flag' => '1'))->select();
		$span=M('main_page')->where(array('etype' => 'span', 'flag' => '1'))->select();
		$quest=M('question')->where(array('flag'=>'1'))->find();
    ...
    public function index($uid=null,$tab='order')
    {
        if(empty($uid)){
            $uid=is_login();
        } else{
		...
		$user_info['vip']=M('vipcard')->where(array('uid'=>$uid))->find();  //vip用户判定
		$this->getExpandInfo($uid);
		/*
		$expandinf = new MobApi();
		$data=$expandinf->expandinfo_select($uid);
		*/
        ...
        switch($tab){
            ...
            case 'userdata':
				...
				$this->getExpandInfo($uid);
			   // $this->getExpandInfo(); //load info
				//$this->_profile_group_list($uid);
                break;
            case 'focus':
     ...
     * 获取所有头衔信息
     */
    private function _order($uid){
        //报名课程
		
		$order = new MobApi();
		$data=$order->get_orderinfo($uid,0); //0 报名
		$data2=$order->get_orderinfo($uid,1); //收费确认
		$data3=$order->get_orderinfo($uid,2); //签到确认
	    $this->assign("comfirmorder", $data2);
	    $this->assign("pass", $data3);
		return $data;
	...
	//	dump($_POST);
       	foreach($_POST as $key=>$value){	
			...
			$map_profile['field_id']=$key;
		//是否 存在
			
			$cnt=M('field')->where($map_profile)->count();
		//	dump($map_profile);
				
					if ($cnt == 0){		
            
					$map_profile['field_data']=$value;
					M('field')->add($map_profile);
					$res=$res+'['+M()->getLastSql()+'] ';
					unset($map_profile['field_data']);
					$ajaxReturn['sql']=$res;
					$ajaxReturn['status']=1;
					$ajaxReturn['oper']='add';
					...
					M('field')->where($map_profile)->save($data);
					$res=$res+'['+M()->getLastSql()+'] ';
				//	dump(M()->getLastSql());
					$ajaxReturn['sql']=$res;
					$ajaxReturn['status']=1;
					$ajaxReturn['oper']='update';
	...
	//	$map_profile['count'] =12;
		$map_profile['status'] =0;
		$res='';
		$ajaxReturn['status']=0;
		$cnt=0;
		...
		$vip=M('vipcard')->where($map_profile)->count();
		//	dump($map_profile);
		//	dump(M()->getLastSql());
		...
		if ($cnt == 1){		
				...
				$ajaxReturn['sql']=$res;
				$ajaxReturn['status']=1;
				$ajaxReturn['oper']='have';
				$ajaxReturn['msg']='你已经递交过申请!';
					
		}else if( $vip==1){
				$cnt=M('vipcard')->where($map_profile)->field('count')->find();
			   	$ajaxReturn['sql']=$res;
				$ajaxReturn['status']=1;
				$ajaxReturn['oper']='have';
				$ajaxReturn['msg']='你已经是VIP还剩余'.$cnt['count'].'次';
		
		
		
		}else{      $data=$map_profile;
					$data['count'] =12;
					$data['status'] =0;
					$data['title'] ='VIP金卡';
    ...
    //    dump($following);exit;
	 ...
	 /**扩展信息分组列表获取  : Indexcontroller.class.php
     * @return mixed
     * @author hongtang
	 **************************************************************************************************************************************
     */
	...
	**************************************************************************************************************************************
     */
        ...
        if(empty($res)){
            $res = M('checkin')->where(array('uid'=>$uid,'create_time'=>array('egt',$time)))->find();
