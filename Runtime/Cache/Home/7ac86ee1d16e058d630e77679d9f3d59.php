<?php if (!defined('THINK_PATH')) exit();?><div id="sub_nav">
    <nav class="navbar navbar-default" role="navigation">
        <div class="container" style="width:1180px;">
            <a href="<?php echo U($MODULE_INFO['entry']);?>" class="navbar-brand logo" ><i class="icon-<?php echo ($brand["icon"]); ?>"></i> <?php if(empty($brand["title"])): echo ($MODULE_ALIAS); else: echo ($brand["title"]); endif; ?></a>
                    ...
                    <?php if($menu_list['right'] != null): ?><ul class="nav navbar-nav navbar-right">
                            <?php if(is_array($menu_list["right"])): $i = 0; $__LIST__ = $menu_list["right"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i; $class=($current==$menu['tab']?'active':''); ?>
                                <?php switch($menu["type"]): case "button": ?><a href="<?php echo ($menu["href"]); ?>" class="<?php echo ($menu["a_class"]); ?>"><i class="icon-<?php echo ($menu["icon"]); ?>"></i><?php echo ($menu["html"]); ?></a><?php break;?>
                                    <?php case "search": ?><form class="navbar-form navbar-right" action="<?php echo ($menu["action"]); ?>"  method="<?php echo ($menu["from_method"]); ?>" role="search">
                                            <div class="search-input-group">
                                                <button type="submit" class="input-btn" style="border-color: transparent;background: transparent;padding: 0 10px 0 10px;"><i class="icon-search"></i></button>
                                                <input type="text" class="input" placeholder="<?php echo ($menu["input_title"]); ?>" name="<?php echo ($menu["input_name"]); ?>" value="<?php echo ($search_keywords); ?>">
                                            </div>
                                            </span>
                                        </form><?php break;?>
                                    <?php default: ?>
